    ...
    for (int i = Hmin; i < Hmax; i++) {
        for (int k = Wmin; k < Wmax; k++) {
            if (pixels[i * height + k * width] > threshold)
                pixels[i * height + k * width] =
                    SDL_MapRGB(SDL_PIXELFORMAT_RGB888, 0, 0, 0);
            else
                pixels[i * height + k * width] =
                    SDL_MapRGB(SDL_PIXELFORMAT_RGB888, 255, 255, 255);
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    SDL_Window *window = SDL_CreateWindow(
        "Image", 0, 0, 640, 400, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    if (window == NULL)
        errx(EXIT_FAILURE, "%s", SDL_GetError());
