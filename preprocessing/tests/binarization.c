...
/*void binarize_square(Uint32* pixels, int Hmin, int Wmin, int Hmax, int Wmax,
...
void binarize_square(SDL_PixelFormat *format, Uint32 *pixels, int threshold,
                     int Hmin, int Wmin, int Hmax, int Wmax, int w, int h) {
    for (int x = Wmin; x < Wmax; x++) {
        for (int y = Hmin; y < Hmax; y++) {
            Uint8 r, g, b;
...
void binarize_surface(SDL_Surface *surface) {
    ...
    for (int k = 1; k < 6; k++) {
        for (int j = 1; j < 6; j++) {
            int threshold = otsu(surface, h_list[j - 1], w_list[k - 1],
                                 h_list[j], w_list[k], w, h);
            binarize_square(surface->format, pixels, threshold, h_list[j - 1],
                            w_list[k - 1], h_list[j], w_list[k], w, h);
        }
    }
}
