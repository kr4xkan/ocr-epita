...
double set_gtk_image_from_surface(GtkImage *img_container, SDL_Surface *surface,
    ...
    // create pixbuf
    pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, has_alpha, 8, surface->w,
                            surface->h);
    ...
    SDL_ConvertPixels(surface->w, surface->h, src_format, surface->pixels,
                      surface->pitch, dst_format, pixels, rowstride);
    ...
    GdkPixbuf *pxbscaled =
        gdk_pixbuf_scale_simple(pixbuf, nW, nH, GDK_INTERP_BILINEAR);
...
void on_save_button(GtkWidget *widget) {
    ...
    GtkWidget *dialog = gtk_message_dialog_new(
        solved, flags, GTK_MESSAGE_INFO, GTK_BUTTONS_CLOSE,
...
int main() {
    ...
    opening_window =
        GTK_WINDOW(gtk_builder_get_object(builder, "opening_window"));
    fixed_container_1 =
        GTK_WIDGET(gtk_builder_get_object(builder, "fixed_container_1"));
    ...
    manual_rotation =
        GTK_WINDOW(gtk_builder_get_object(builder, "manual_rotation"));
    ...
    next_button_rot =
        GTK_BUTTON(gtk_builder_get_object(builder, "next_button_rot"));

    GtkScale *angle_slider =
        GTK_SCALE(gtk_builder_get_object(builder, "angle_entry"));
    ...
    fixed_container_bin =
        GTK_FIXED(gtk_builder_get_object(builder, "fixed_container_bin"));
    bin_image = GTK_IMAGE(gtk_builder_get_object(builder, "bin_image"));
    binarize_check =
        GTK_TOGGLE_BUTTON(gtk_builder_get_object(builder, "binarize_check"));
    next_button_bin =
        GTK_BUTTON(gtk_builder_get_object(builder, "next_button_bin"));

    next_button_cutter =
        GTK_BUTTON(gtk_builder_get_object(builder, "next_button_cutter"));
    ...
    GtkWindow *line_check_window =
        GTK_WINDOW(gtk_builder_get_object(builder, "line_check_window"));
    GtkDrawingArea *area =
        GTK_DRAWING_AREA(gtk_builder_get_object(builder, "area"));
    GtkImage *img_lines =
        GTK_IMAGE(gtk_builder_get_object(builder, "img_lines"));
    GtkButton *next_lines =
        GTK_BUTTON(gtk_builder_get_object(builder, "next_lines"));

    AppState app_state = {.line_check_window = line_check_window,
                          .img_lines = img_lines,
    ...
    fixed_container3 =
        GTK_FIXED(gtk_builder_get_object(builder, "fixed_container3"));
    ...
    GtkButton *return_button =
        GTK_BUTTON(gtk_builder_get_object(builder, "return_button"));
    rect.x = 15;
    rect.y = 15;
    rect.h = 40;
    rect.w = 40;
    ...
    // signals
    // WINDOW
    g_signal_connect(opening_window, "destroy", G_CALLBACK(gtk_main_quit),
                     NULL);
    g_signal_connect(manual_rotation, "destroy", G_CALLBACK(gtk_main_quit),
                     NULL);
    ...
    g_signal_connect(line_check_window, "destroy", G_CALLBACK(gtk_main_quit),
                     NULL);
    //
    // OPEN
    //
    ...
    g_signal_connect(next_button_rot, "clicked", G_CALLBACK(on_next_button_rot),
                     NULL);
    ...
    g_signal_connect(GTK_RANGE(angle_slider), "value-changed",
                     G_CALLBACK(on_rotate), &app_state);
    //
    // BIN
    //
    g_signal_connect(binarize_check, "toggled", G_CALLBACK(on_binarize_check),
                     NULL);
    g_signal_connect(next_button_bin, "clicked", G_CALLBACK(on_next_button_bin),
                     &app_state);
    ...
    g_signal_connect(area, "button-press-event", G_CALLBACK(on_press),
                     &app_state);
    g_signal_connect(area, "button-release-event", G_CALLBACK(on_release),
                     &app_state);
    g_signal_connect(area, "motion-notify-event", G_CALLBACK(on_cursor_motion),
                     &app_state);

    g_signal_connect(next_lines, "clicked", G_CALLBACK(on_next_button_cutter),
                     &app_state);
    ...
    g_signal_connect(return_button, "clicked", G_CALLBACK(on_return_home),
                     NULL);
